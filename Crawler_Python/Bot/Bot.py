...
def run() -> dict[str, list[str]]:
    ...
    for tipo_produto in Tipo_Produto:
        ...
        for chave in data:
            if chave == "Loja":
                data[chave] += [Lojas.KABUM.name for i in range(len(data_k["Nome"]))] + [Lojas.MERCADO_LIVRE.name for i in range(len(data_ml["Nome"]))]
            else:
                data[chave] += data_k[chave] + data_ml[chave]
